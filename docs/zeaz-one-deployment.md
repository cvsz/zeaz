...
| `https://www.zeaz.dev/products/zeaz-one` | `http://127.0.0.1:18082` for local preview | Optional path-specific redirect Worker |

All application origins bind only to loopback. Terraform creates proxied tunnel
CNAMEs only for `one.zeaz.dev` and `support.zeaz.dev`. It deliberately does not
create, import or replace the existing `api.zeaz.dev` DNS record. Instead, a
specific Workers route handles only `/v1/products/zeaz-one*`; all other shared
API routes remain untouched.

The optional `www` route also leaves the existing corporate Worker intact. Its
more-specific route redirects only the ZEAZ One product path to the primary
product hostname.
...
To add the canonical corporate product redirect as well:
...
`MANAGE_TUNNEL_CONFIG=false` is the safe default. Terraform manages DNS and
Workers routes, but an operator must merge only the `one.zeaz.dev` and
`support.zeaz.dev` entries from
...
`api.zeaz.dev` to this tunnel fragment; it remains the existing shared API.
