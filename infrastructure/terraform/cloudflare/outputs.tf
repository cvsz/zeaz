...
output "chat_url" {
  ...
  description = "Public NextChat URL after the proxied DNS record and tunnel ingress are active."
}
...
output "cloudflared_ingress" {
  value = concat(
    [
      ...
      { hostname = var.uperfect_hostname, service = var.uperfect_origin },
      { hostname = var.zok_hostname, service = var.zok_origin },
      { hostname = var.z_spark_hostname, service = var.z_spark_origin },
    ],
