...
function render() {
  ...
  $('#payments').innerHTML=payments.length?payments.map(payment=>`<article class="menu-item"><div><h4>${payment.id} · ${payment.status}</h4><p>${payment.provider_order_id||'รอ SCB order ID'} · ${money(payment.amount)}</p><p><small>${new Date(payment.created_at).toLocaleString('th-TH')}</small></p></div>${payment.status==='pending'?`<button class="text-button" data-payment="${payment.id}">ตรวจสอบกับ SCB</button>`:''}</article>`).join(''):'<p>ยังไม่มี SCB QR payment</p>';
