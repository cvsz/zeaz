...
def ai_provider_keys() -> dict[str,str]:
    ...
    return {name:value for name,value in parsed.items() if name in {"gemini","nvidia","zai","openrouter","opencode","groq","byteplus"} and isinstance(value,str) and value.strip()}

def ai_public_config() -> dict:
    keys=ai_provider_keys()
    return {"enabled":bool(keys) or hf_enabled(),"providers":{"gemini":bool(keys.get("gemini")),"nvidia":bool(keys.get("nvidia")),"zai":bool(keys.get("zai")),"opencode":bool(keys.get("opencode")),"openrouter":bool(keys.get("openrouter")),"groq":bool(keys.get("groq")),"byteplus":bool(keys.get("byteplus")),"huggingface":hf_enabled()},"catalog":"live","chat_only":True}
...
def ai_catalog() -> dict:
    ...
    with AI_MODEL_LOCK:
        ...
        for name, loader in (("gemini",gemini_models),("nvidia",nvidia_models),("zai",zai_models),("opencode",opencode_models),("openrouter",openrouter_models),("groq",groq_models),("byteplus",byteplus_models)):
