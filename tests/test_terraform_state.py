...
class TerraformStateTests(unittest.TestCase):
    ...
    def test_migration_is_fail_closed_and_verifies_remote_state(self):
        ...
        self.assertIn('sha256sum "$(basename "$backup")"', script)
        ...
        self.assertIn('"$backend_initialized" != "true"', script)
        self.assertIn("Remote Terraform state lineage differs", script)
        self.assertIn("Local Terraform state must not be group/world accessible", script)
        self.assertIn('state list -state="$local_state"', script)
        self.assertIn('install -d -m 700 "$BACKUP_DIR"', script)
        self.assertLess(
            script.index('state list -state="$local_state" > "$before_addresses"'),
    ...
    def test_plan_does_not_silently_use_remote_backend(self):
        ...
        self.assertIn("Cloudflare environment must be a regular file", script)
        self.assertIn("Cloudflare environment must not be group/world accessible", script)
        self.assertNotIn("init -backend=false", script)

    def test_migration_rejects_group_readable_local_state_before_init(self):
        with TemporaryDirectory() as temporary:
            ...
            state.write_text(
                '{"version":4,"lineage":"test-lineage","resources":[{"type":"example"}]}\n',
                encoding="utf-8",
            )
            state.chmod(0o640)
            ...
            result = subprocess.run(
                ...
                env={
                    **os.environ,
                    ...
                    "CLOUDFLARE_STATE_BACKUP_DIR": str(root / "backups"),
                },
                text=True,
                capture_output=True,
                check=False,
            ...
            self.assertFalse((stack / "backend.tf").exists())
    ...
    def test_migration_control_flow_with_isolated_fake_backend(self):
        with TemporaryDirectory() as temporary:
            ...
            (stack / "terraform.tfstate").chmod(0o600)
            (stack / "backend.r2.tf.example").write_text(
                (
                    ROOT
                    / "infrastructure/terraform/cloudflare/backend.r2.tf.example"
...
case " $* " in
  ...
  *" state pull "*) printf '{"version":4,"lineage":"test-lineage","resources":[{"type":"cloudflare_dns_record"}]}\\n' ;;
...
""",
            ...
            checksum = Path(str(state_backups[0]) + ".sha256")
            self.assertTrue(checksum.is_file())
            self.assertEqual(checksum.stat().st_mode & 0o777, 0o600)
            self.assertTrue(checksum.read_text().rstrip().endswith(state_backups[0].name))
            self.assertIn("migrate verification passed", result.stdout)

    def test_backend_is_retained_after_successful_init_if_verification_fails(self):
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            stack = root / "stack"
            stack.mkdir()
            ...
            local_state.write_text(
                '{"version":4,"lineage":"local-lineage","resources":[{"type":"example"}]}\n',
                encoding="utf-8",
            )
...
set -Eeuo pipefail
case " $* " in
  *" state list -state="*) printf 'cloudflare_dns_record.moopiew\\n' ;;
  *" state list "*) printf 'cloudflare_dns_record.moopiew\\n' ;;
  *" state pull "*) printf '{"version":4,"lineage":"wrong-lineage","resources":[{"type":"cloudflare_dns_record"}]}\\n' ;;
  *" init -migrate-state -force-copy "*) exit 0 ;;
  *) exit 2 ;;
esac
""",
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("lineage differs", result.stderr)
            self.assertTrue((stack / "backend.tf").is_file())
