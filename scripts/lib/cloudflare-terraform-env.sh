...
cloudflare_load_terraform_env() {
  ...
  export TF_VAR_llmwiki_hostname="${LLMWIKI_HOSTNAME:-llmwiki.zeaz.dev}"
  export TF_VAR_llmwiki_origin="${LLMWIKI_ORIGIN:-http://127.0.0.1:5173}"
  export TF_VAR_zksato_hostname="${ZKSATO_HOSTNAME:-zksato.zeaz.dev}"
  export TF_VAR_zksato_origin="${ZKSATO_ORIGIN:-http://127.0.0.1:9569}"
  ...
  export TF_VAR_cmeerp_origin="${CMEERP_ORIGIN:-http://127.0.0.1:8000}"
